...
Subject : CSCI 114 Programming Assignment 4
Commands to execute it: g++ CSci114_PA4.cpp 
                        ./a.out 6 7
*/
...
// finding a system if its in safe or not
			...
			int allot[][M])
{
    ...
    // Mark all Nrocesses as infinish
	bool finish[N] = {0};
...

{
  ...
  while (hold != safeSeq[ProcessesRun])
    ...
    cout<<"   :Allocation Vector is: "<<endl;
    cout<<endl;
    for(int i=0; i<M; i++)
    { cout<<alloc[hold][i];
    }
   ...
   srand(time(NULL));
{
 ...
 ifstream inputfile1("/Users/patron/Downloads/C++ program/requests.txt");    //make sure to change the location according to the file's lcoation
